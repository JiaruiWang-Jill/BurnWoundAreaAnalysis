...
# @Last Modified time: 2019-08-08 16:06:18
import cv2 as cv 
import numpy as np 
import sys
import math
...
def getRealCoordinateList():
	patternPoints = []
	for i in range(0,10):
		patternPoints.append([0, 2.5+1.0*i, 0])

	for i in range(0,10):
		patternPoints.append([1.0, 2.5+1.0*i, 0])

	for i in range(0,10):
		patternPoints.append([1.5+1.0*i, 1.0, 0])

	for i in range(0,10):
		patternPoints.append([1.5+1.0*i, 0, 0])
...
def findContourPoints():
	...
	cv.namedWindow('img', cv.WINDOW_NORMAL)
	cv.resizeWindow('img', int(img.shape[1]/resizeAspect), int(img.shape[0]/resizeAspect))
	cv.setMouseCallback('img', on_mouse3)
	while(1):
		cv.imshow('img', img)
...
def getPixelCoordinateListByMouse():
	...
	# 	# Use shi tomasi algorithm to auto detect the corners
	# 	ROI = gray[min_y:min_y+height, min_x:min_x+width]
	...
	# 	points = np.int0(points)
	# 	corners = []
	# 	print("Shi Tomasi corner detector ..., points are", points)
	# 	img2 = img.copy()
	# 	for i in points:
	# 		x, y = i.ravel()
	# 		corners.append((min_x+x, min_y+y))
	# 		cv.circle(img2, (min_x+x, min_y+y), 10, (0,0,255), -1 )
	# 	cv.namedWindow('with points', cv.WINDOW_NORMAL)
	# 	cv.resizeWindow('with points', int(height/resizeAspect), int(width/resizeAspect))
	# 	cv.imshow('with points', img2)
	# 	cv.waitKey(0)
	# 	return corners
		# Manually click the points
...
def testAreaCalculation(homography):
	...
	print("real\t","calculated\t\t","error\t")
	print("totalError is", totalError)
	for result in output:
		print(result[0],"\t", result[1],"\t", result[2],"%\t")
	print("average error is", math.sqrt(totalError)/len(output))
...
def main():
	...
	print("transfer the img into calibrated ones ...")
	calibratedImg = undistortImage(img, camera_matrix, dist_coefs)
